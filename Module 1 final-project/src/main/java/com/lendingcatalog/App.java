...
public class App {
    ...
    private void initialize() {
        ...
        try {
             List<String> memberDataList = FileStorageService.readContentsOfFile( FILE_BASE_PATH +"members.dat");
             for(String line : memberDataList){
                 String[] memberFields = line.split(FIELD_DELIMITER);
                 if (memberFields.length == 3){
                     String firstName = memberFields[0];
                     String lastName = memberFields[1];
                     String itemsFile = memberFields [2];
                     Member member = new Member(firstName, lastName);
                     List<CatalogItem> memberItems = new ArrayList<>();
                     List<String> memberItemLines = FileStorageService.readContentsOfFile( FILE_BASE_PATH + itemsFile);
                     for(String itemLine : memberItemLines){
                         String[] memberItemFields = itemLine.split(FIELD_DELIMITER);
                         if (memberItemFields.length == 4){
                             String itemType = memberItemFields[0];
                             if(itemType.equals("book")){
                                 String title = memberItemFields[1];
                                 String author = memberItemFields[2];
                                 String date = memberItemFields[3];
                                 Book book = new Book(title, author, LocalDate.parse(date));
                                 book.registerItem();
                                 memberItems.add(book);

                             } else if(itemType.equals("movie")){
                                 String movieTitle = memberItemFields[1];
                                 String director = memberItemFields[2];
                                 String date = memberItemFields[3];
                                 Movie movie = new Movie(movieTitle, director, LocalDate.parse(date));
                                 movie.registerItem();
                                 memberItems.add(movie);
                             }else if(itemType.equals("tool")){
                                 String type = memberItemFields[1];
                                 String manufacture = memberItemFields[2];
                                 String count = memberItemFields[3];
                                 Tool tool = new Tool(type, manufacture, Integer.parseInt(count));
                                 tool.registerItem();
                                 memberItems.add(tool);
                             }else{
                                 throw new Exception("Expected book, movie, or tool.");
                             }
                         }else{
                             throw new Exception("Expected 4 fields in member items file.");
                         }
                     }
                     catalog.put(member.toString(), memberItems);
                 }else{
                     throw new Exception("Expected 3 fields in members file.");
                 }
             }
        } catch (Exception e) {
            e.printStackTrace();
